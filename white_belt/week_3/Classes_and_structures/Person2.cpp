...
private:
    ...
    string concatOutput(vector<string>& parts)
    {
    ...
    string formatNamesVector(const vector<string>& names)
    {
        if (names.empty())
            return "";
        string res = "(";
    ...
    vector<string> getHistory(const map<int, string>& changes, int year, string curr_name)
    {
    ...
    vector<string> resolveFullName(const string& first_name, const string& last_name)
    {
    ...
    string getLastKnownName(const map<int, string>& changes, int year)
    {
        string name = "";
...
int main() {
  Person person;
  person.ChangeLastName(1909, "Ivanov");
